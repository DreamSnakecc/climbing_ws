...
class BodyPlanner(object):
    ...
    def __init__(self):
        ...
        self.stride_length_m = float(
            rospy.get_param("/swing_leg_controller/stride_length_m", 0.0)
        )
        self.fixed_stride_mode = self.gait == "sequential" and self.stride_length_m > 0.0
        if self.fixed_stride_mode and any(abs(value) > 1e-9 for value in self.linear_velocity_world):
            rospy.logerr(
                "body_planner: fixed stride mode ignores non-zero linear_velocity_world_mps; "
                "body advance is generated by support-leg targets"
            )
            self.linear_velocity_world = [0.0, 0.0, 0.0]
    ...
    def _support_mask_sequential(self, elapsed_s):
        ...
        State machine:
          ...
          SWING_ACTIVE -> ALL_HOLD : swing leg returned to support; legacy mode may time out
        """
        ...
        if self._gait_state == "SWING_ACTIVE":
            ...
            timed_out = (not self.fixed_stride_mode) and swing_elapsed >= self.swing_timeout_s
            if self.fixed_stride_mode and swing_elapsed >= self.swing_timeout_s:
                rospy.logwarn_throttle(
                    3.0,
                    "body_planner: fixed stride swing %s still waiting after %.1fs; "
                    "holding until adhesion controller confirms support",
    ...
    def _compute_swing_progression(self):
        ...
        """
        if (not self.fixed_stride_mode
                and self._gait_state == "SWING_ACTIVE"
                and self._swing_start_time is not None):
